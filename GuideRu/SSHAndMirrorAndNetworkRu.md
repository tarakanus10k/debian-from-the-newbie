...
## 2. Настройка зеркала
Воспользуемся командой `nano /etc/apt/sources.list` и видим, что в этом файле находится только одна строка, закомментируем ее (перед этой строкой нужно поставить #). После этого переходим на сайт Debian-зеркалов и оттуда копируем нужные нам зеркала ([ссылка](https://www.debian.org/mirror/list)). 
